/*************************************************************************
	...
	Modifications made Oleg [oleg.rybalko] marked accordingly
...
// [oleg.rybalko, 04/17-2009] use table
//		res +=  "<a href='#' class='" + opts.classNames.title + "'" + tooltip + ">" + this.data.title + "</a>";
//		return res;

		var intTableWidth = 390 - 32;
		
		var intLevel = parseInt(this.data.level);
		if (intLevel > 0) {
			intTableWidth -= 16*intLevel;
		}
		
		var strTable = "<table class='inlineTable' style='width:" + intTableWidth + "px;'><tr>";
		res += strTable;
		res += "<td id='col1' " + tooltip + ">";
		
		this.data.title = this.data.title.replace(/^\s*/, '').replace(/\s*$/, '');	
		var intTitleWidthMax = this.tree.options.maxWidth;
		if (this.data.level != "root") {
			intTitleWidthMax -= 16 * this.data.level;
		}
		
		var strTitle = this.data.title;
		var intTitleWidth = getWidth(strTitle);
		while(intTitleWidth > intTitleWidthMax) {
			strTitle = strTitle.replace(/^\s*/, '').replace(/\s*$/, '');	
			strTitle = strTitle.substr(0, strTitle.length - 1);
			strTitle = strTitle.replace(/^\s*/, '').replace(/\s*$/, '');	
			intTitleWidth = getWidth(strTitle);
		}
		if (this.data.title.length > strTitle.length) {
			strTitle += "&hellip;";
		}
		...
		if (this.data.red && this.data.yellow && this.data.green) {
			res += "<div class='indicator'>";
			res += "<div id=green>" + this.data.green + "</div>&nbsp;";
			res += "<div id=yellow>" + this.data.yellow + "</div>&nbsp;";
			res += "<div id=red>" + this.data.red + "</div>";
			res += "</div>";
		} else if (this.data.green) {
			res += "<div class='indicator'>";
			res += "<div id=green>&nbsp;</div>&nbsp;";
			res += "<div id=clear>&nbsp;</div>&nbsp;";
			res += "<div id=clear>&nbsp;</div>";
			res += "</div>";			
		} else if (this.data.yellow) {
			res += "<div class='indicator'>";
			res += "<div id=clear>&nbsp;</div>&nbsp;";
			res += "<div id=yellow>&nbsp;</div>&nbsp;";
			res += "<div id=clear>&nbsp;</div>&nbsp;";
			res += "</div>";			
...
// [oleg.rybalko, 07/06/2009] end code injection		
		return res;
	},
...
//		this.tree.logDebug("dtnode.onClick(" + event.type + "): dtnode:" + this + ", button:" + event.button + ", which: " + event.which);
		...
		//	this.span.getElementsByTagName("a")[0].focus();
			this.span.getElementsByTagName("td")[0].focus();
		}
...
DynaTree.prototype = {
	...
	reloadAjax: function() {
		...
		// [oleg.rybalko, 02/18-2010] make sure "persist" is enabled before accessing the data; add if below
		if( opts.persist ) {
